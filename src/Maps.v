...
Proof.
  ...
  rewrite <- beq_id_false_iff in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
Lemma t_update_shadow : forall A (m: total_map A) v1 v2 x,
  ...
  = t_update m x v2.
Proof.
  intros A m v1 v2 x. apply functional_extensionality. intros y.
  unfold t_update. destruct (beq_id x y). reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros [x] [y]. destruct (beq_id (Id x) (Id y)) eqn:H.
  - apply ReflectT. unfold beq_id in H. destruct (string_dec x y) in H.
    + rewrite e. reflexivity.
    + inversion H.
  - apply ReflectF. unfold beq_id in H. destruct (string_dec x y).
    + inversion H.
    + unfold not. unfold not in n. intros H'. inversion H'. apply n in H1. inversion H1.
Qed.
(** [] *)
...
Proof.
  ...
  unfold t_update. destruct (beq_id x y) eqn:H.
  - generalize H. destruct x as [x]. destruct y as [y]. unfold beq_id.
